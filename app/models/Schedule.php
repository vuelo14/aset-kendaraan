...
class Schedule {
    ...
    {
        $pdo = Database::getInstance()->pdo();
        $sql = "SELECT s.*, v.plat, v.merk, v.foto_path, v.current_responsible, v.jenis as jenis_kendaraan FROM tax_schedule s JOIN vehicles v ON v.id=s.vehicle_id ORDER BY s.next_date ASC";
        return $pdo->query($sql)->fetchAll();
    }
